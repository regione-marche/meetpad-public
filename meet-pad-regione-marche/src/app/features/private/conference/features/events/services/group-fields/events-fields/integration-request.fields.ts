...
import { IntegrationRequest } from '@features/private/conference/core';
...
export function integrationRequest(
    ...
    mode: ActionForm,
    model: IntegrationRequest = new IntegrationRequest()
): Map<string, FormFieldGroup> {
    const gf: Map<string, FormFieldGroup> = new Map();
    this.formGroupsFields.set(EventType.INTEGRATION_REQUEST, gf);

    if (!(model instanceof IntegrationRequest))
        model = new IntegrationRequest(model);
    ...
    switch (mode) {
        case ActionForm.CREATE:
            ...
            this.createPanelDocument(gf,model,EventType.INTEGRATION_REQUEST)
