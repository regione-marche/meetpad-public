...
import { OpinionExpress } from '@features/private/conference/core';
import { EventGroupFieldsService } from '../event-group-fields.service';

import { EventAttach } from '../../../../../core';

export function opinionExpress(
    this: EventGroupFieldsService,
    mode: ActionForm,
    model: OpinionExpress = new OpinionExpress()
): Map<string, FormFieldGroup> {
    const gf: Map<string, FormFieldGroup> = new Map();
    this.formGroupsFields.set(EventType.OPINION_EXPRESS, gf);

    if (!(model instanceof OpinionExpress))
        model = new OpinionExpress(model);
    ...
    switch (mode) {
        case ActionForm.CREATE:
            ...
            this.createPanelDocument(gf,model,EventType.OPINION_EXPRESS);
            
            this.asyncPopulateDefaultIntegration(gf, EventType.OPINION_EXPRESS);
    ...
    if (!environment.customConfigurationConf.determinationTypeEventView) {
        const fields = gf.get('document').fields;
        fields.delete('determinationType');
        fields.get('category').size = '12|6|6';
        fields.get('protocolNumber').size = '12|6|6';
        fields.get('protocolDate').size = '12|6|6';
    }
