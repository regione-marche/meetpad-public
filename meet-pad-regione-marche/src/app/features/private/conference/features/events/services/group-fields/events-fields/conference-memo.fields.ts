...
import { ConferenceMemo } from '@features/private/conference/core';
import { EventGroupFieldsService } from '../event-group-fields.service';

import { EventAttach } from '../../../../../core';

export function conferenceMemo(
    this: EventGroupFieldsService,
    mode: ActionForm,
    model: ConferenceMemo = new ConferenceMemo()
): Map<string, FormFieldGroup> {
    const gf: Map<string, FormFieldGroup> = new Map();
    this.formGroupsFields.set(EventType.CONFERENCE_MEMO, gf);

    if (!(model instanceof ConferenceMemo))
        model = new ConferenceMemo(model);
    ...
    switch (mode) {
        case ActionForm.CREATE:
            ...
            this.createPanelDocument(gf,model,EventType.CONFERENCE_MEMO);
