...
import conferenza.DTO.RispostaConferenzaAutorizzazioniDTO;
...
public class ConferenceController {
	...
	/**
	 * Restituisce le autorizzazioni per la conferenza
	 * @param id
	 * @param locale
	 * @return
	 */	
	@GetMapping("/conferences/{id}/permissions")
	@ApiOperation(value = "permissions for conference identified by 'id'")
	public RispostaConferenzaAutorizzazioniDTO autorizzazioniConferenza(@PathVariable int id) {
		LOGGER.debug("Class: ConferenceController - Method: autorizzazioniConferenza");
		ConferenzaAutorizzazioniDTO autorizzazioni = this.confservice.getAutorizzazioniConferenza(id);
		RispostaConferenzaAutorizzazioniDTO risposta = new RispostaConferenzaAutorizzazioniDTO();
		risposta.setData(autorizzazioni);
		return risposta;
	}
	...
	public RispostaIdentifiableDTO patchConferenza(@RequestBody ConferenzaDTO conferenceDTO, @PathVariable Integer id) {
		LOGGER.debug("Class: ConferenceController - Method: patchConferenza");
		Conferenza conferenza = this.confservice.patchConferenza(conferenceDTO, id);
		
		Evento eventoModificaRichiedente = confservice.patchFlagAbilitaModificaRichiedente(conferenceDTO, conferenza);
		
		/*
		 * l'invio delle mail (essendo asincrono) deve essere chiamato solo dopo la
		 * conclusione della modifica della conferenza e quindi nel
		 * controller
		 */
		if (eventoModificaRichiedente != null) {
			eventoService.doEventoModificaRichiedente(eventoModificaRichiedente);
		}
	 ...
	 * @return
	 */
	@RequestMapping(value = "/conferences/indizione/{idconferenza}/{idDocumento}/{mail}", method = RequestMethod.GET)
	public RispostaNoData indizioneMail(
			@PathVariable int idconferenza, @PathVariable int idDocumento, @PathVariable String mail) {

		String subject = "Indizione conferenza";
		String from = "prova@eng.it";
		String textmessage = "Messaggio di Prova \n procedere all'autoaccreditamento  ";
		Partecipante partecipante = null;
		Documento documento = documentoRepository.findById(idDocumento).get();
		mailerRepository.sendMailIndizioneForConferenceMailSpecifica(idconferenza, from, textmessage,
					subject, null, documento, mail, partecipante);
