CREATE OR REPLACE VIEW cdst.mailer_indizione
AS SELECT (pc.id_partecipante || '-'::text) || pp.id_persona AS id,
    cc.fk_stato,
    pp.id_persona,
    pp.nome,
    pp.cognome,
   ...
   FROM accreditamento acc
     ...
     JOIN tipologia_conferenza_specializzazione tc ON tc.codice::text = cc.fk_tipologia_conferenza_specializzazione::text
     JOIN comune cm ON cm.codice::text = cc.fk_comune_sessione_simultanea::text
     JOIN provincia pv ON pv.codice::text = cc.fk_provincia_sessione_simultanea::text
     JOIN ente ammpr ON ammpr.id_ente = cc.fk_ente_procedente
  WHERE acc.flag_accreditato = true;

-- Permissions

ALTER TABLE cdst.mailer_indizione OWNER TO cdst;
GRANT ALL ON TABLE cdst.mailer_indizione TO cdst;
