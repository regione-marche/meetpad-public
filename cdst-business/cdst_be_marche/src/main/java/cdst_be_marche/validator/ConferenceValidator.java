...
import cdst_be_marche.repository.EventoCalendarioDataObbligatoriaRepository;
...
public class ConferenceValidator {
		...
		if (definizione != null) {
			if (definizione.getIstanza() != null) {
				...
				if (istanza.getTipologiaConferenza() != null) {
					List<EventoCalendarioDataObbligatoria> listaDateObbligatorie = this.dataObblRepo
							.findByTipologiaConferenza(
									this.tipoConfRepo.findById(istanza.getTipologiaConferenza().getKey()).get());
	...
	public void validateTypeConference(String type) {
		if (type.isEmpty() || type == null) {
			throw new NotFoundEx("La tipologia conferenza è obbligatoria");
		}		
	}
