...
import cdst_be_marche.controller.DocumentsController;
import cdst_be_marche.util.model.TokenJWT;
import cdst_be_marche.util.model.TokenJWT_min;
import cdst_be_marche.util.model.TokenWSO2JWT;
import cdst_be_marche.util.model.TokenWSO2JWT_SUAP;
...
public class JWTsService {
	...
	public String getCodiceFiscaleFromToken(HttpServletRequest request) {
		...
		if (token != null) {
			return getCodiceFiscaleFromInput(token);
		}
		else {
			token = request.getHeader(CUSTOM_AUTHORIZATION);
			if (token != null) {
				return getCodiceFiscaleFromAssertionInput(token, tokenWSO2JWTClass);
			}
		}
		return null;
	}
	...
	public void dumpJWT(String pJwtToken) {
		...
		LOGGER.info("Header =  " + decodedJwt.getHeader());
		LOGGER.info("Algorithm =  " + decodedJwt.getAlgorithm());
		LOGGER.info("Audience =  " + decodedJwt.getAudience());
		decodedJwt.getClaims().forEach((k, v) -> {
			LOGGER.info("Claim " + k + " = " + v.asString());
		});
		LOGGER.info("ContentType =  " + decodedJwt.getContentType());
		LOGGER.info("ExpiresAt =  " + decodedJwt.getExpiresAt());
		LOGGER.info("Id =  " + decodedJwt.getId());
		LOGGER.info("Issuer =  " + decodedJwt.getIssuer());
	...
	private boolean verifyToken(String pJwtToken) throws Exception {
		...
		if (pJwtToken != null) {
			...
			LOGGER.info(modulus.toString());
			BigInteger exponent = generatePublic.getPublicExponent();
			LOGGER.info(exponent.toString());
			test = true;
		}
		return test;
	}
