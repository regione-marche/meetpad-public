CREATE OR REPLACE VIEW cdst.view_submit_to_external_protocol AS
 SELECT DISTINCT rd.id AS id_registro,
   ...
   FROM cdst.registro_documento rd
     ...
     JOIN cdst.tipo_evento te ON te.codice = ee.fk_tipo_evento
     JOIN cdst.observer_registry rr ON rr.subrscribed_event_type = te.codice
     LEFT JOIN cdst.observer_registry_audit rra ON rra.id_evento = ee.id_evento
